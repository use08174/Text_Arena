...
[출력 형식]
승자: [캐릭터 이름]  
이유: [전투 상황을 묘사한 완전한 문장]. [판정 기준에 따른 승리 이유를 설명하는 완전한 문장].

출력 형식은 반드시 위와 같아야 하며,
승자는 이름만 제시하고, 이유는 RPG 전투 장면처럼 생생하게 묘사된 문장 두 개로 구성하되, 각각은 주어와 술어를 갖춘 완전한 문장이어야 한다.
이유의 첫 문장은 전투 상황을 설명하고 두 번째 문장은 판정기준에 따라서 승패 이유를 명확하게 설명하라.
