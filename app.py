...
만약 기준과 캐릭터 설명이 직접적으로 일치하지 않더라도, AI가 가진 언어 이해 능력을 활용해 판정 기준에 맞는 캐릭터 특성을 합리적으로 해석하고, 그에 따라 승자를 결정하라.
...
[출력 형식]  
승자: [캐릭터 이름]  
이유: [판정 기준이 녹아든 전투 상황을 묘사한 한 문장]. [그 전투에서 드러난 캐릭터의 특성이나 전략이 기준과 자연스럽게 연결되며 승리를 이끈 이유를 설명한 문장].
"""
)
